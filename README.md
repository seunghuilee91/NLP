# NLP
> 시소러스 기반 기법 (수작업, wordnet)  <Br>
> 통계 기반 기법 (말뭉치)  <Br>
> 추론 기반 기법 (word2vec)  <Br>
---
- WordNet 등의 시소러스를 이용하면 유의어를 얻거나 단어 상이의 유사도를 측정하는 등 유용한 작업을 할 수 있다.
- 시소러스 기반 기법은 시소러스를 작성하는 데 엄청난 인적 자원이 든다거나 새로운 단어에 대응하기 어렵다는 문제가 있다. 
- 현재는 말뭉치를 이용해 벡터화하는 방식이 주로 쓰인다.
...
- 단어의 벡터 공간에서는 의미가 가까운 단어는 그거리도 가까울 것으로 기대된다.
